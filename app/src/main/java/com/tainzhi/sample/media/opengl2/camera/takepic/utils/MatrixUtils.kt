...
object MatrixUtils {
    ...
    fun getMatrix(mvpMatrix: FloatArray?, type: Int, imgWidth: Int, imgHeight: Int, viewWidth: Int,
                  viewHeight: Int) {
        if (imgHeight > 0 && imgWidth > 0 && viewWidth > 0 && viewHeight > 0) {
            val projection = FloatArray(16)
            ...
            if (type == TYPE_FITXY) {
                ...
                Matrix.multiplyMM(mvpMatrix, 0, projection, 0, viewMatrix, 0)
            }
            val sWhView = viewWidth.toFloat() / viewHeight
            val sWhImg = imgWidth.toFloat() / imgHeight
            if (sWhImg > sWhView) {
                when (type) {
                    TYPE_CENTERCROP -> Matrix.orthoM(projection, 0, -viewWidth/2f, viewWidth/2f, -viewHeight/2f, viewHeight/2f, 1f, 3f)
            ...
            Matrix.scaleM(modelMatrix, 0, viewWidth/2f, viewWidth/2f /( imgWidth.toFloat() /(imgHeight .toFloat())), 1f)
            Matrix.setLookAtM(viewMatrix, 0, 0f, 0f, 1f, 0f, 0f, 0f, 0f, 1f, 0f)
