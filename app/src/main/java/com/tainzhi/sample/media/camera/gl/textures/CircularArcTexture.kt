...
class CircularArcTexture(val center: Vertex3F, val radius: Float, val startAngle: Float, val sweepAngle: Float): Texture() {
    ...
    override fun onDraw() {
        ...
        // 每个顶点3个值，xyz. 此处获取顶点数
        GLES20.glDrawArrays(GLES20.GL_LINE_STRIP, 0, vertices.size / COORDS_PER_VERTEX)
    ...
    private fun generateVertices() {
        ...
        vertices = FloatArray((segments + 1) * COORDS_PER_VERTEX)
        val angleStep = sweepAngle / segments.toFloat()
        for (i in 0 .. segments) {
            val angleInRadius = (startAngle + angleStep * i)/180f * PI
            vertices[i * COORDS_PER_VERTEX] = (center.x + radius * cos(angleInRadius)).toFloat()
            vertices[i * COORDS_PER_VERTEX + 1] = (center.y + radius * sin(angleInRadius)).toFloat()
            vertices[i * COORDS_PER_VERTEX + 2] = 0f
        }
    }
